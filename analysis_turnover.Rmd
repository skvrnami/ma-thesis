...
p+geom_point(aes(color=loyal)) + labs(title="Position of candidates in Slovakia, 1998-2016, minor parties", x="candidate's position in t", y="candidate's position in t+1", color="Did the candidate switched between parties?") + theme(legend.position="top") + scale_color_manual(values=c("red", "black"), labels=c("Yes", "No"))
...
```{r}
model1 <- lm(order2 ~ order1 + loyal + minor + origPartyDissolved, data=sk9816ord)
summary(model1)
```

```{r}
model2 <- lm(order2 ~ order1 + loyal*minor + origPartyDissolved, data=sk9816ord)
summary(model2)
```
